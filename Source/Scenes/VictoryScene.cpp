...
//---------------------------------------------------------------------------
//! 項目選択 (キー上下 + Enter/Space。マウスは render 側の ImGui で処理)
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
//! 背景 + ImGui 直描きメニュー (ホバー・クリックの判定もここ)
//---------------------------------------------------------------------------
void VictoryScene::render()
{
    ...
    for (int i = 0; i < m_menuItemCount; i++)
    {
        ...
        if (isSelected)
        {
            drawList->AddText(textPos, IM_COL32(255, 217, 0, 255), displayText);
        }
        else
        {
